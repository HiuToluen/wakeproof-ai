...
export async function createChallengeAttempt(attempt) {
  ...
  await database.withTransactionAsync(async () => {
    ...
    await database.runAsync('UPDATE alarm_sessions SET challenge_attempt_count = challenge_attempt_count + 1, updated_at = ? WHERE id = ?', [new Date().toISOString(), attempt.sessionId]);
  });
