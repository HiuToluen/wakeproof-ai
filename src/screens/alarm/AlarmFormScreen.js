...
import { createAlarm, getAlarmById, updateAlarm } from '../../database/alarmRepository';
import { colors, spacing, typography } from '../../theme';

const defaults = { title: 'Wake up', hour: '07', minute: '00', repeatDays: [], isEnabled: true, snoozeDuration: '5', maxSnooze: '2', challengeMode: CHALLENGE_MODES.RANDOM };
...
export default function AlarmFormScreen({ navigation, route }) {
  ...
  const validate = () => {
    ...
    if (!Number.isInteger(snooze) || snooze < 1 || snooze > 15) return 'Snooze duration must be between 1 and 15 minutes.';
  ...
  const save = async () => {
    ...
    const alarm = { ...form, title: form.title.trim(), hour: Number(form.hour), minute: Number(form.minute), snoozeDuration: Number(form.snoozeDuration), maxSnooze: Number(form.maxSnooze), createdAt: createdAt || now, updatedAt: now };
    try {
      if (alarmId) await updateAlarm(alarmId, alarm);
      else await createAlarm({ ...alarm, id: `${Date.now()}-${Math.random().toString(36).slice(2, 10)}` });
      navigation.goBack();
  ...
  return (
    <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.flex}>
      <ScreenContainer scroll>
        ...
        <View style={styles.switchRow}><Text style={styles.label}>Enabled</Text><Switch onValueChange={(value) => updateField('isEnabled', value)} value={form.isEnabled} /></View>
