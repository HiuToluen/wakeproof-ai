...
export default function ChallengePreviewScreen({ navigation, route }) {
  ...
  useEffect(() => {
    ...
    const back = BackHandler.addEventListener('hardwareBackPress', () => { navigation.navigate(preview ? 'PreviewCameraChallenge' : 'CameraChallenge', { alarmId, sessionId, preview, challenge }); return true; });
  ...
  const submit = async () => {
    ...
    try {
      ...
      navigation.navigate(preview ? 'PreviewChallengeVerification' : 'ChallengeVerification', { alarmId, sessionId, preview, challenge, image: compressedImage });
    } catch (error) {
      setSubmitting(false);
