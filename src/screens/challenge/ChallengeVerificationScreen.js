...
import { ActivityIndicator, Alert, BackHandler, Keyboard, KeyboardAvoidingView, Platform, Pressable, StyleSheet, Text, TextInput, TouchableWithoutFeedback, Vibration, View } from 'react-native';
...
import { prepareChallengeAlerts, reconcileChallengeScreenSession, stopChallengeAlerts, returnChallengeToRinging } from '../../services/challengeFlowService';
...
export default function ChallengeVerificationScreen({ navigation, route }) {
  ...
  const stateRef = useRef('VERIFYING');
  ...
  const timeout = useCallback(async (message, reason = 'timeout') => {
    if (timedOut.current) return;
    timedOut.current = true;
    requestId.current += 1;
    inFlight.current = false;
    stateRef.current = 'TIMED_OUT';
    if (mounted.current) setState('TIMED_OUT');
    if (holdTimer.current) clearInterval(holdTimer.current);
    if (preview) navigation.popTo('AlarmPreview', { alarmId });
    else {
      await returnChallengeToRinging(sessionId, reason).catch((error) => Alert.alert('Unable to restart alarm', error.message));
  ...
  const completeChallenge = useCallback(async (verification, currentRequest) => {
    ...
    if (!preview) {
      ...
      await releaseAlarmStreamVolumeSnapshot({ hasNextRinging: Boolean(nextSession) });
  ...
  const runVerification = useCallback(async (options = {}) => {
    ...
    if (!preview) {
      const reconciliation = await reconcileChallengeScreenSession(sessionId, route.name).catch(() => null);
      if (reconciliation?.action !== 'stay-challenge') return;
      if (mounted.current) setChallengeAllowed(true);
    }
    if (getRemaining(challenge.deadlineAt) <= 0) { timeout(); return; }
    ...
    try {
      ...
      if (persistedAttemptCount >= MAX_CHALLENGE_ATTEMPTS) { setLimitReached(true); if (!preview) await registerChallengeAttemptLockout(sessionId); await timeout('Too many failed attempts. The alarm is ringing again. Challenge retry is temporarily locked.', 'lockout'); return; }
      attempt = preview ? null : await createChallengeAttempt({ sessionId, challengeId: currentChallenge.id, imageUri: image.uri });
      const verification = await verifyChallenge({ sessionId, challenge: currentChallenge, image, technicalRetryNumber: serviceRetryCountRef.current });
      ...
      if (verification.isValid) {
        ...
        setRetakeRequired(true);
        if (nextAttemptCount >= MAX_CHALLENGE_ATTEMPTS && !preview) { setLimitReached(true); await registerChallengeAttemptLockout(sessionId); await timeout('Too many failed attempts. The alarm is ringing again. Challenge retry is temporarily locked.', 'lockout'); return; }
        if (nextAttemptCount >= MAX_CHALLENGE_ATTEMPTS) { setLimitReached(true); setErrorMessage('Verification limit reached in preview.'); stateRef.current = 'TECHNICAL_ERROR'; setState('TECHNICAL_ERROR'); return; }
      ...
      if (!mounted.current || timedOut.current || requestId.current !== currentRequest) return;
      const latestSession = preview ? null : await getAlarmSessionById(sessionId);
      const latestDeadlineAt = preview ? challenge.deadlineAt : latestSession?.challengeDeadlineAt;
      const latestLocked = latestSession?.challengeLockedUntil && new Date(latestSession.challengeLockedUntil).getTime() > Date.now();
      ...
      stateRef.current = 'TECHNICAL_ERROR';
      setState('TECHNICAL_ERROR');
    } finally {
      if (requestId.current === currentRequest) {
        inFlight.current = false;
        if (mounted.current && stateRef.current === 'VERIFYING') {
          stateRef.current = 'TECHNICAL_ERROR';
          setErrorMessage('Verification ended without a usable result. Please try again.');
          setState('TECHNICAL_ERROR');
        }
      }
    }
  ...
  useEffect(() => {
    ...
    prepareChallengeAlerts(sessionId, preview).catch(() => {});
    if (!started.current) {
...
const createStyles = ({ colors, spacing, typography, radius }) => StyleSheet.create({
  ...
  title: { ...typography.heading, color: colors.textPrimary, marginTop: spacing.xl, textAlign: 'center' },
  success: { ...typography.heading, color: colors.success, marginTop: spacing.xl, textAlign: 'center' },
  message: { ...typography.body, color: colors.textSecondary, marginTop: spacing.md, textAlign: 'center' },
  warning: { ...typography.body, color: colors.danger, marginTop: spacing.md, textAlign: 'center' },
  button: { marginTop: spacing.md },
  holdButton: { alignItems: 'center', backgroundColor: colors.primary, borderRadius: radius.pill, height: 140, justifyContent: 'center', marginTop: spacing.lg, alignSelf: 'center', width: 140 },
  holdText: { ...typography.heading, color: colors.onPrimary },
  mathRow: { alignItems: 'center', flexDirection: 'row', justifyContent: 'center', marginTop: spacing.md },
  mathQuestion: { color: colors.textPrimary, fontSize: 28, fontWeight: '700', minWidth: 130 },
  input: { borderColor: colors.border, borderRadius: radius.sm, borderWidth: 1, color: colors.textPrimary, fontSize: 28, fontWeight: '700', minWidth: 120, padding: spacing.md, textAlign: 'center' },
});
